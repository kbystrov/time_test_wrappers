...
static unsigned long *setup_shared_var(void)
{
	...
	fd = open(".tmp_mmap", O_RDWR|O_CREAT|O_TRUNC, 0700);
	BUG_ON(fd == -1);
...
#else
	__asm__ __volatile__("movq $0,%0; rep; nop" : "=g"(*flag) :: "memory");
#endif
}
...
usage:
	...
	int err_getres_code = clock_getres(CLOCK_MONOTONIC, &monotonic_res);

	if(err_getres_code){
		#ifdef __STDC_LIB_EXT1__
    	size_t error_len = strerrorlen_s(err_getres_code) + 1;
    	char error_buf[error_len];
    	strerror_s(error_buf, error_len, err_getres_code);
    	fprintf(stderr, "PID = %ld; Error message: %s\n", me, error_buf);
		#else
		char error_buf[MSG_LEN];
        int error = strerror_r (err_getres_code, error_buf, MSG_LEN);
		...
		switch (error) {
            case EINVAL:
                    ...
                    break;
            case ERANGE:
                    fprintf (stderr, "strerror_r() failed: buffer too small: %ld\n", MSG_LEN);
					break;
            case 0:
	...
	printf("CLOCK_MONOTONIC resolution of PID = %d is: TV_SEC = %ld; TV_NSEC = %ld;\n", me, monotonic_res.tv_sec, monotonic_res.tv_nsec);
	fflush(stdout);
